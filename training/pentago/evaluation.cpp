...
void Write_evaluate() {
	...
	for (int j = 0;j < 36;j++) {
		for (int i = 0;i < 729;i++) fwrite(&row1[j][i], sizeof(float), 1, fp);
		for (int i = 0;i < 729;i++) fwrite(&row2[j][i], sizeof(float), 1, fp);
		for (int i = 0;i < 729;i++) fwrite(&row3[j][i], sizeof(float), 1, fp);
		for (int i = 0;i < 729;i++) fwrite(&srant1[j][i], sizeof(float), 1, fp);
		for (int i = 0;i < 243;i++) fwrite(&srant2[j][i], sizeof(float), 1, fp);
	}
